...
@router.get(
    "/", response_description="List all addresses", response_model=List[Address]
)
...
@router.post(
    "/",
    response_description="Create a new address",
...
@router.get(
    "/{id}", response_description="Get a single address", response_model=Address
)
def get_address(id: str, request: Request):
    ...
    raise HTTPException(
        status_code=status.HTTP_404_NOT_FOUND, detail=f"Address with ID {id} not found"
    )
...
def delete_all_addresses(request: Request, response: Response):
    ...
    raise HTTPException(
        status_code=status.HTTP_404_NOT_FOUND, detail=f"Address not found"
    )
